...
public class 백준_G5_1068_트리 {
	...
	public static void main(String[] args) throws IOException{
		...
		for(int i=0;i<N;i++) {
			...
			if(parent==-1) { // root는 부모가 없어서 -1
				root =i;
			}else {
				list[parent].add(i); //각 노드 자식을 add
			}
		}
	...
	static void DFS(int x,int parent) { //위치한 노드, 부모노드
		if(list[x].isEmpty()) { //자식이 없는 경우 =>단말
			answer[x]=1; 
		}
		
		for(int y:list[x]) {
			DFS(y,x);
			answer[x]+=answer[y]; //위에 if문으로 1이 생기므로 부모로 누적시킨다. (자식 노드 없는데까지 방문해서 무모에 단말 노드 개수 누적)
		}
	}
